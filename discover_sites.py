...
# --- SETUP, UTILITY, & SAVING FUNCTIONS (CORRECTED) ---
def setup_google_sheet():
    try:
        ...
        if not leads_ws.get_all_values():
            leads_ws.append_row(["URL", "Email", "Phone Number", "Facebook", "Instagram", "Twitter", "LinkedIn", "Scraped Timestamp"])
        try:
            logs_ws = spreadsheet.worksheet("Run Logs")
            if logs_ws.get("A1").first() != "Timestamp":
                 logs_ws.insert_row(["Timestamp", "Total Leads Found", "Credits Used", "Summary"], 1)
...
def get_existing_urls_from_sheet(worksheet):
    try:
        ...
        logging.error(f"❌ Could not fetch existing URLs: {e}"); return set()
...
def save_to_gsheet(worksheet, lead_data):
    try:
        ...
        logging.error(f"❌ GSheet API error: {e}"); time.sleep(60)
...
def save_log_to_gsheet(worksheet, total_leads, credits_used, summary_lines):
    try:
        ...
        logging.error(f"❌ Could not save run summary: {e}")
...
def analyze_site(url):
    ...
    try:
        ...
        logging.warning(f"   [FAIL] Could not access site: {e}"); return None
    except Exception as e:
        logging.error(f"   [FAIL] Unexpected analysis error: {e}"); return None
...
if __name__ == "__main__":
    ...
    if not leads_ws or not logs_ws:
        exit(1)
    ...
    for phrase_obj in phrases_to_process:
        ...
        for result in results:
            if url := result.get('link'):
                if cleaned_url := clean_and_validate_url(url):
    ...
    logging.info(f"\n--- Analyzing {len(new_urls_to_check)} new sites. ---")
    ...
    with open(SEARCH_CONFIG_FILE, 'w') as f:
        json.dump(final_phrase_list, f, indent=2)
    ...
    log_summary.append(f"📈 Total new leads: {total_leads_found}")
    log_summary.append(f"💳 API Credits Used: {credits_used} ({api_calls_made} searches)")
