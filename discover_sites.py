...
# --- (All imports and helper functions remain the same) ---
...
def setup_google_sheet():
    try:
        ...
        gc, spreadsheet = gspread.service_account(filename=GOOGLE_CREDS_FILE), gc.open(GOOGLE_SHEET_NAME)
    ...
    for phrase_obj in phrases_to_process:
        ...
        for result in results:
            if url := result.get('link'):
                if cleaned_url := clean_and_validate_url(url): all_potential_urls.add(cleaned_url)
